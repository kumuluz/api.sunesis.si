...
const StudyComponent = props => {
  ...
  let listItems = []
  if (items !== undefined) {
    listItems = items.map((point, index) => <li key={index}>{point}</li>)
  }
  ...
  if (features !== undefined) {
    listFeatures = features.map((feature, index) => (
      <div key={index} className="col-md-4 study-feature">
    ...
    <div className="container">
      <div id={props.id} className="row content">
        <div className="col-lg-12">
          <p className="studies-title font-content-title">{props.title}</p>
